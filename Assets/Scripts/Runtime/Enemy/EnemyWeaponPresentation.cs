...
{
    /// <summary>
    /// Adds a short muzzle beat to the enemy's successful shot event.
    /// EnemyWeapon owns cadence and projectile rules; this component only
    /// makes the already-committed shot readable in the vertical slice.
    /// </summary>
    [RequireComponent(typeof(EnemyWeapon))]
    public sealed class EnemyWeaponPresentation : MonoBehaviour
    {
        [SerializeField] Transform firePoint;
        [SerializeField, Min(0f)] float flashDuration = 0.09f;
        [SerializeField, Min(0f)] float flashLength = 0.28f;
        [SerializeField, Min(0f)] float flashWidth = 0.095f;
        [SerializeField, Min(0f)] float sideFlashLength = 0.18f;
        [SerializeField, Min(0f)] float sideFlashWidth = 0.1f;
        [SerializeField] Color flashColor = new Color(0.45f, 1f, 0.55f, 1f);
        [SerializeField] Color flashEdgeColor = new Color(0.05f, 1f, 0.16f, 0f);
        [SerializeField] int sortingOrder = 76;
        [SerializeField, Min(0f)] float cameraShakeStrength = 0.014f;
        [SerializeField, Min(0f)] float cameraShakeDuration = 0.04f;
        ...
        Vector2 flashDirection = Vector2.left;
        ...
        void EnsureFlashLines()
        {
            ...
            flashMaterial = RuntimeVisualMaterial.Create("Enemy Muzzle Flash");
            coreFlash = CreateFlashLine("Enemy Muzzle Flash Core");
            upperFlash = CreateFlashLine("Enemy Muzzle Flash Upper");
            lowerFlash = CreateFlashLine("Enemy Muzzle Flash Lower");
        }
        ...
        void SetFlashVisible(bool visible, float alpha)
        {
            ...
            SetLine(coreFlash, origin - direction * 0.03f, tip, flashWidth * eased, visible, alpha);
            ...
            float alpha)
        {
            ...
            float z = transform.position.z - 0.04f;
