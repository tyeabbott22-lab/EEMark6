...
namespace ExtraterrestrialExhaust.Player
{
    ...
    public sealed class PlayerFlightInput : MonoBehaviour
    {
        ...
        [Tooltip("Supplies Q/E rotation and C stabilization when an action asset does not include those optional desktop bindings.")]
        [FormerlySerializedAs("includeEe5KeyboardFallback")]
        [SerializeField] bool includeKeyboardFallback = true;
        ...
        void Update()
        {
            ...
            if (canReadInput && includeKeyboardFallback && Keyboard.current != null)
            {
                ...
                // Digital keys are independent booleans: a held diagonal keeps
                // full thrust and full torque, while gamepad input remains
                // analog.
                if (turnKeyPressed)
                {
                ...
                // Stabilization is exclusive. S/C must win over W/Space rather
                // than algebraically cancelling to neutral.
            ...
            if (!canReadInput)
            {
                ...
                || (includeKeyboardFallback
                    && Keyboard.current != null
                    && Keyboard.current.xKey.wasPressedThisFrame))
            {
                // Keep X as a direct desktop flip binding even when the action
                // asset has a Flip action but no serialized X binding.
