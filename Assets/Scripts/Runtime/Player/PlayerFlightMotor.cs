...
namespace ExtraterrestrialExhaust.Player
{
    ...
    public sealed class PlayerFlightMotor : MonoBehaviour
    {
        ...
        [FormerlySerializedAs("enforceEe5Profile")]
        [SerializeField] bool applySliceTuning = true;
        ...
        void ApplySliceTuning()
        {
            thrustForce = Ee5SliceProfile.ThrustForce;
            // Keep the vertical-slice response in one tuning source rather
            // than splitting it between scene and prefab overrides.
            rotationTorque = Ee5SliceProfile.PlayerRotationTorque;
        ...
        void FixedUpdate()
        {
            ...
            // Keep the turn-response strategy explicit so tuning remains
            // intentional rather than emerging from competing force writers.
            bool useTurnResponse = Ee5SliceProfile.PlayerTurnResponseOwnsRotation;
            if (!useTurnResponse)
            ...
            {
                if (turning)
                    ApplyTurnResponse(command.x);
                else if (turnReleaseTimer >= Ee5SliceProfile.PlayerReleaseBrakeDelay)
                    ApplyReleaseBrake();
            }
            ...
            float maxAngularVelocity = Ee5SliceProfile.PlayerMaxAngularVelocity;
        ...
        void ApplyTurnResponse(float inputAmount)
        {
            float targetAngularVelocity = -Mathf.Clamp(inputAmount, -1f, 1f)
                * Ee5SliceProfile.PlayerMaxAngularVelocity;
        ...
        void ApplyReleaseBrake()
        {
            body.angularVelocity = Mathf.MoveTowards(
                body.angularVelocity,
                0f,
                Ee5SliceProfile.PlayerReleaseAngularBrake
                * Time.fixedDeltaTime);
        }

        void ApplyDirectPhysicsAssist(
            Vector2 command,
            bool thrusting,
            bool turning,
            bool applyRotationTorque)
        {
            if (!Ee5SliceProfile.EnableDirectFlightAssist
                || !body.simulated
                || body.bodyType != RigidbodyType2D.Dynamic)
                return;
