...
{
    /// <summary>
    /// Centralized authored tuning for the playable vertical slice. Runtime
    /// systems can opt out for controlled experiments, while the shipped route
    /// keeps one source of truth instead of scattered inspector literals.
    /// </summary>
    public static class Ee5SliceProfile
    {
        // Keep physics on the craft root; the enlarged child visual must not
        // change body mass, collider size, or flight resistance.
        ...
        // Keep the physics hurtbox on the root body so visual scale never
        // changes the craft's gameplay footprint.
        public const float PlayerHitboxRadius = 0.112f;
        public static readonly Vector2 PlayerHitboxOffset =
            new Vector2(-0.004f, 0f);

        // The motor consumes these values as the baseline desktop flight
        // response; optional direct-input assistance stays explicit below.
        ...
        // The flight motor consumes this explicit torque value so gameplay
        // tuning stays centralized rather than split across scene overrides.
        public const float PlayerRotationTorque = RotationTorque;
        // Unity's default Rigidbody2D cap is 7 radians/second. Keep the
        // equivalent degree value when the motor applies an explicit clamp.
        public const float PlayerMaxAngularVelocity = 400f;
        public const float PlayerTurnAcceleration = 2880f;
        // Brake released turn momentum without rotating the craft upright.
        // Deliberate flips remain free while neutral input settles quickly.
        public const float PlayerReleaseAngularBrake = 2160f;
        public const float PlayerReleaseBrakeDelay = 0.04f;
        // A short turn input should become readable immediately while a held
        // turn still permits deliberate flips.
        public const bool PlayerTurnResponseOwnsRotation = true;
        public const bool EnableDirectFlightAssist = false;
        // Stabilization is an explicit input. Leaving neutral correction off
        // prevents a release tap from fighting an intentional turn or flip.
        ...
        // Boosted exhaust needs a visibly stronger cue without changing the
        // craft's physical footprint.
        ...
        // These root-space anchors keep exhaust attached to the visual craft.
