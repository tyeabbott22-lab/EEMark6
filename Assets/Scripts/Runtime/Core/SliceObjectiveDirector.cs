...
namespace ExtraterrestrialExhaust.Core
{
    ...
    /// <summary>
    /// Owns the vertical-slice objective sequence.
    ...
    public sealed class SliceObjectiveDirector : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            // every scene object has finished initialization so the HUD cannot
            // briefly report CLEAR ENCOUNTER before its key, gate, and exit
            // references have settled.
            Refresh();
        ...
        public void Refresh()
        {
            ...
            else if (exit && exit.IsUnlocked)
            {
                ...
                // it never directs the player into a locked portal.
        ...
        void ReportReferenceRecovery()
        {
            ...
            Debug.LogWarning(
                "Slice objective flow has an incomplete serialized contract: "
                + details
                + ". Assign the missing scene references before shipping this level.",
                this);
        }
